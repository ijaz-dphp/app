...
function me_recruitment_scripts() {
    ...
    // Theme Styles
    wp_enqueue_style('theme-styles', get_template_directory_uri() . '/css/theme-styles.css', array(), filemtime(get_template_directory() . '/css/theme-styles.css'));
    
    // Page Specific Styles
    if (is_page_template('page-about.php')) {
        wp_enqueue_style('about-styles', get_template_directory_uri() . '/css/about-styles.css', array(), filemtime(get_template_directory() . '/css/about-styles.css'));
    }
    if (is_page_template('page-candidate.php')) {
        wp_enqueue_style('candidate-styles', get_template_directory_uri() . '/css/candidate-styles.css', array(), filemtime(get_template_directory() . '/css/candidate-styles.css'));
    }
    if (is_page_template('page-jobs.php')) {
        wp_enqueue_style('jobs-styles', get_template_directory_uri() . '/css/jobs-styles.css', array(), filemtime(get_template_directory() . '/css/jobs-styles.css'));
    }
    ...
    // Common JS
    wp_enqueue_script('common-js', get_template_directory_uri() . '/js/common.js', array('jquery'), '1.0.1', true);
    
    // Localize script for AJAX
    wp_localize_script('common-js', 'recruitmentAjax', array(
        'ajaxurl' => admin_url('admin-ajax.php'),
        'nonce' => wp_create_nonce('candidate_cv_nonce')
    ));
}
...
function me_recruitment_customize_register($wp_customize) {
    ...
    // Submit CV Button Text
    $wp_customize->add_setting('submit_cv_button_text', array(
        'default'           => 'Submit Your CV',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    
    $wp_customize->add_control('submit_cv_button_text', array(
        'label'    => __('Submit CV Button Text', 'me-recruitment'),
        'section'  => 'me_recruitment_contact',
        'type'     => 'text',
    ));
...
// AJAX Handler for Candidate CV Submission
add_action('wp_ajax_submit_candidate_cv', 'me_recruitment_ajax_process_candidate_submission');
add_action('wp_ajax_nopriv_submit_candidate_cv', 'me_recruitment_ajax_process_candidate_submission');

function me_recruitment_ajax_process_candidate_submission() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'candidate_submissions';
    ...
    } elseif (!is_email($_POST['candidate_email'])) {
    ...
    if ($_FILES['cv_file']['error'] !== UPLOAD_ERR_OK) {
        ...
        switch ($_FILES['cv_file']['error']) {
            ...
            case UPLOAD_ERR_FORM_SIZE:
                $error_message = 'File size exceeds limit (5MB max)';
                break;
    ...
    if (!in_array($file_extension, $allowed_extensions)) {
        wp_send_json_error(array('message' => 'Invalid file type. Only PDF, DOC, and DOCX files are allowed.'));
    }
    
    // Validate file size (5MB max)
    $max_size = 5 * 1024 * 1024;
    if ($_FILES['cv_file']['size'] > $max_size) {
        wp_send_json_error(array('message' => 'File size exceeds limit. Maximum file size is 5MB.'));
    }
    ...
    if (!file_exists($cv_dir)) {
        wp_mkdir_p($cv_dir);
        file_put_contents($cv_dir . '/.htaccess', 'deny from all');
    }
    ...
    $file_path = $cv_dir . '/' . $cv_file_name;
    
    // Move uploaded file
    if (!move_uploaded_file($_FILES['cv_file']['tmp_name'], $file_path)) {
        wp_send_json_error(array('message' => 'Failed to upload file. Please try again.'));
    }
    ...
    if ($result === false) {
        error_log('Candidate submission database error: ' . $wpdb->last_error);
        wp_send_json_error(array('message' => 'Database error. Please try again or contact support.'));
    }
    ...
    $message_body .= "Thank you for submitting your application for the position of " . $job_title . ".\n\n";
    $message_body .= "We have received your CV and will review it shortly. If your qualifications match our requirements, we will contact you for the next steps.\n\n";
    ...
    $admin_subject = 'New Candidate Application - ' . $job_title;
    $admin_message = "A new candidate has submitted an application.\n\n";
    $admin_message .= "Name: " . $candidate_name . "\n";
    $admin_message .= "Email: " . $candidate_email . "\n";
    $admin_message .= "Phone: " . $candidate_phone . "\n";
    $admin_message .= "Job Title: " . $job_title . "\n";
    if ($applied_job_title) {
        $admin_message .= "Applied for: " . $applied_job_title . "\n";
    }
    $admin_message .= "CV: " . $cv_file_name . "\n\n";
    $admin_message .= "View all candidates in the admin panel.";
    
    wp_mail($admin_email, $admin_subject, $admin_message);
    
    wp_send_json_success(array('message' => 'Thank you for your application! We have received your CV and will contact you soon.'));
}

// Process candidate form submission (keep for backward compatibility)
function me_recruitment_process_candidate_submission() {
    // Check if form was submitted
    if (!isset($_POST['submit_cv']) && !isset($_POST['submit_cv_sidebar'])) {
        return;
    }
    ...
    if (!isset($_POST['candidate_cv_nonce']) || !wp_verify_nonce($_POST['candidate_cv_nonce'], 'submit_candidate_cv')) {
        echo '<div class="alert alert-danger" style="background: #f8d7da; color: #842029; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
            <h3 style="margin-top: 0;">✗ Security Check Failed</h3>
    ...
    // Validate required fields
    if (empty($_POST['candidate_name']) || empty($_POST['job_title']) || empty($_POST['candidate_email']) || empty($_POST['candidate_phone'])) {
        echo '<div class="alert alert-danger" style="background: #f8d7da; color: #842029; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
            <h3 style="margin-top: 0;">✗ Missing Required Fields</h3>
            <p>Please fill in all required fields (Name, Job Title, Email, Phone).</p>
        </div>';
        return;
    }
    ...
    if (!isset($_FILES['cv_file']) || $_FILES['cv_file']['error'] === UPLOAD_ERR_NO_FILE) {
        echo '<div class="alert alert-danger" style="background: #f8d7da; color: #842029; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
            <h3 style="margin-top: 0;">✗ CV File Required</h3>
            <p>Please upload your CV file (PDF, DOC, or DOCX).</p>
        </div>';
        return;
    }
    ...
    if (isset($_FILES['cv_file']) && $_FILES['cv_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!move_uploaded_file($_FILES['cv_file']['tmp_name'], $cv_file_path)) {
            echo '<div class="alert alert-danger" style="background: #f8d7da; color: #842029; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
                ...
                <p>Failed to upload CV file. Please check file permissions and try again.</p>
            </div>';
            return;
        }
        ...
        if (isset($_FILES['cv_file']['error'])) {
            switch ($_FILES['cv_file']['error']) {
                ...
                case UPLOAD_ERR_CANT_WRITE:
                    ...
                    break;
                default:
                    $error_message = 'File upload error (Code: ' . $_FILES['cv_file']['error'] . ')';
            }
        }
        echo '<div class="alert alert-danger" style="background: #f8d7da; color: #842029; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
    ...
    if ($result) {
        ...
        error_log('Candidate submission failed: ' . $db_error);
        ...
        echo '<div class="alert alert-danger" style="background: #f8d7da; color: #842029; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
            ...
            <p style="font-size: 0.9em; margin-top: 10px;"><strong>Error:</strong> Database insertion failed.</p>
        </div>';
    }
}

// Add debug function for testing (remove in production)
function me_recruitment_debug_submission() {
    if (current_user_can('manage_options') && isset($_GET['debug_form'])) {
        global $wpdb;
        $table_name = $wpdb->prefix . 'candidate_submissions';
        
        echo '<div style="background: #fff3cd; padding: 20px; margin: 20px; border-left: 4px solid #ffc107;">';
        ...
        $upload_dir = wp_upload_dir();
        $cv_dir = $upload_dir['basedir'] . '/candidate-cvs';
        $cv_dir_exists = file_exists($cv_dir);
        $cv_dir_writable = is_writable($cv_dir);
        
        echo '<p><strong>CV Directory exists:</strong> ' . ($cv_dir_exists ? '✅ Yes' : '❌ No') . '</p>';
        echo '<p><strong>CV Directory writable:</strong> ' . ($cv_dir_writable ? '✅ Yes' : '❌ No') . '</p>';
        echo '<p><strong>CV Directory path:</strong> ' . esc_html($cv_dir) . '</p>';
        ...
        echo '<p><strong>Total submissions:</strong> ' . intval($count) . '</p>';
        
        echo '</div>';
    }
}
add_action('wp_footer', 'me_recruitment_debug_submission');
